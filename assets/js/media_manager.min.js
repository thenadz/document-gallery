...
{silent:!0});b.observe(c);this.editLibrary=c;a.controller.Library.prototype.activate.apply(this,arguments)}});a.view.Settings.dg=a.view.Settings.extend({className:"collection-settings dg-settings",template:a.template("dg-settings")});var e=wp.media.view.MediaFrame.Post;wp.media.view.MediaFrame.Post=e.extend({initialize:function(){e.prototype.initialize.apply(this,arguments);this.states.add([new a.controller.Library({id:"dg",title:d.dgMenuTitle,priority:50,toolbar:"main-dg",filterable:"all",multiple:"add",
...
multiple:!0}));this.controller.setState("dg-edit");this.controller.modal.focusManager.focus()}})},dgEditToolbar:function(){var b=this.state().get("editing");this.toolbar.set(new a.view.Toolbar({controller:this,items:{insert:{style:"primary",text:b?d.updatedg:d.insertdg,priority:80,requires:{library:!0},click:function(){var b=this.controller,a=b.state();b.close();wp.media.editor.insert(wp.media.dg.shortcode(a.get("library")).string().replace(/\sdgorder=/ig," order=").replace(/\sdgorderby=/ig," orderby="));
b.setState(b.options.state);b.reset()}}}}))},dgAddToolbar:function(){this.toolbar.set(new a.view.Toolbar({controller:this,items:{insert:{style:"primary",text:d.addTodg,priority:80,requires:{selection:!0},click:function(){var b=this.controller,a=b.state();b.state("dg-edit").get("library").add(a.get("selection").models);a.trigger("reset");b.setState("dg-edit")}}}}))}});wp.media._dgDefaults={id:wp.media.view.settings.post&&wp.media.view.settings.post.id,columns:dgDefaults.columns,fancy:dgDefaults.fancy,
relation:dgDefaults.relation,limit:dgDefaults.limit,mime_types:dgDefaults.mime_types,post_status:dgDefaults.post_status,post_type:dgDefaults.post_type,attachment_pg:dgDefaults.attachment_pg,descriptions:dgDefaults.descriptions,new_window:dgDefaults.new_window,dgorder:dgDefaults.order,dgorderby:dgDefaults.orderby};wp.media.dgDefaults=wp.media.view.settings.dgDefaults?g.extend({},wp.media._dgDefaults,wp.media.view.settings.dgDefaults):wp.media._dgDefaults;wp.media.dg=new wp.media.collection({tag:"dg",
editTitle:wp.media.view.l10n.editdgTitle,defaults:wp.media.dgDefaults,setDefaults:function(b){var a=this,f=!g.isEqual(wp.media.dgDefaults,wp.media._dgDefaults);g.each(this.defaults,function(d,e){b[e]=a.coerce(b,e);d!==b[e]||f&&d!==wp.media._dgDefaults[e]||delete b[e]});return b}})})(jQuery,_);
(function(h,g,d,a){base={state:[],edit:function(a,b){var c=this.type,f=d[c].edit(a.replace(/\sorder=/ig," dgorder=").replace(/\torderby=/ig," dgorderby="));this.pausePlayers&&this.pausePlayers();_.each(this.state,function(a){f.state(a).on("update",function(a){b(d[c].shortcode(a).string(),"dg"===c)})});f.on("close",function(){f.detach()});f.open()}};h=_.extend({},base,{state:["dg-edit"],template:d.template("editor-dg"),initialize:function(){var a=d.dg.attachments(this.shortcode,d.view.settings.post.id),
b=this.shortcode.attrs.named,c=this;a.more().done(function(){a=a.toJSON();_.each(a,function(a){dgThumbnails.hasOwnProperty(a.id)?a.thumbnail={url:dgThumbnails[a.id],width:dgThumbnails.width,height:dgThumbnails.height}:a.sizes&&(b.size&&a.sizes[b.size]?a.thumbnail=a.sizes[b.size]:a.sizes.thumbnail?a.thumbnail=a.sizes.thumbnail:a.sizes.full&&(a.thumbnail=a.sizes.full))});c.render(c.template({attachments:a,columns:b.columns?parseInt(b.columns,10):d.dgDefaults.columns}))}).fail(function(a,b){c.setError(b)})}});
